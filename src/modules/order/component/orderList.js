...
class OrderList extends React.Component {
        ...
        if (res.file.status === 'done') {
            this.setState({
                ...
                if(response.success){
                    ...
                    Message.error(response.backMsg);
                }
            });
        }
    }

    handleUpdatExpress = res => {
        console.log("res ===", res);
        if (res.file.status === 'done') {
            this.setState({
                showUpdatExpress: false,
    ...
    //打开更新快递状态Modal
    showUpdatExpressModal = () => {
        this.setState({
            showUpdatExpress: !this.state.showUpdatExpress
        });
    }
    ...
    render() {
        ...
        return (
            <div className="zui-content page-orderList">
                        ...
                        extra={sessionStorage.type !== '3' ? (
                            <ButtonGroup>
                                ...
                                <Button
                                    type="primary"
                                    icon='upload'
                                    onClick={this.showUpdatExpressModal}
                                >更新快递状态</Button>
                            </ButtonGroup>
                        ) : null}
                    >
                ...
                <Modal
                    title="更新快递状态"
                    visible={showUpdatExpress}
                    destroyOnClose
                    onCancel={this.showUpdatExpressModal}
                    footer={null}
                >
                    <Upload
                        headers={{
                        ...
                        onChange={this.handleUpdatExpress}
                    >
                        <Button>
