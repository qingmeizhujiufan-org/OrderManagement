...
const getSenderDataUrl = restUrl.BASE_HOST + 'sender/queryList';
...
class Index extends React.Component {
  ...
  getSenderData = () => {
    let that = this;
    ajax.getJSON(getSenderDataUrl, {}, data => {
      if (data.success) {
        let senderData = data.backData.content;
        if(senderData.length == 0) {
          Message.warning('请添加寄件人信息，否则无法新增订单');
          setTimeout(function () {
            return that.context.router.push('/frame/order/sender');
            }, 2000)
        } else {
          this.setState({
  ...
  render() {
    ...
    const {userId, region, isOperator, senderData, submitProduct, submitLoading, showModal, pagination, loading, allProduct, selectedRowKeys} = this.state;
    const rowSelection = {
      selectedRowKeys,
