...
class Index extends React.Component {
    ...
    this.orderColumns = [
      {
        ...
        render: (text, record, index) => (
          <div>
            <Popconfirm title="是否删除?" onConfirm={() => this.onDelete(record.productId)}>
  ...
  onDelete = id => {
    ...
    proData = proData.filter(item => item.productId !== id);
  ...
  handleOk = () => {
    let {selectedProduct, proData, selectedRowKeys} = this.state;
    ...
    proData = proData.filter(item => includes(selectedRowKeys, item.productId));
  ...
  onSelectChange = (selectedRowKeys, selectedRows) => {
    ...
    if (selectedNum <= 4) {
      this.setState({
        selectedRowKeys: this.state.selectedRowKeys.concat(selectedRowKeys),
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        values.childrenDetail = this.state.proData.map(item => {
          return {
